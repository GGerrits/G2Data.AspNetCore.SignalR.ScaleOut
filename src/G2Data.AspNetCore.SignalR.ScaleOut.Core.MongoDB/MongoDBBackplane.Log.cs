...
internal static partial class MongoDBBackplaneLog
{
    ...
    [LoggerMessage(EventId = 8, Level = LogLevel.Error, Message = "An error occurred while subscribing to MongoDB change stream. Retrying in {reconnectDelayInSeconds} seconds ...")]
    public static partial void CreateChangeStreamFailed(ILogger logger, int reconnectDelayInSeconds, Exception ex);

    [LoggerMessage(EventId = 9, Level = LogLevel.Debug, Message = "Publishing message, scope: {scope}, method: {method}, senderId: {senderId}, sendAt: {sentAt}")]
